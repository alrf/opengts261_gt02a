    ...
    implements DataTransport
{
    ...
    public static void initELogHOSProvider()
    {
        if (Device.elogHosProvider == null) {
            ...
            try {
                ...
                Print.logInfo("Installed ELogHOSProvider: ["+StringTools.className(Device.elogHosProvider)+"] v"+elogVers+" "+elogDesc);
  ...
  //public static final String FLD_maintDescriptionHR0   = "maintDescriptionHR0";   // hours maint #0 description
    ...
    *** @param ndx  The fuel-tank index (#1 or #2)
    *** @return The Fuel tank profile
    **/
    public String getFuelTankProfile(int ndx)
    {
        switch (ndx) {
            case 0 : 
                Print.logWarn("Invalid fuel-tank index: " + ndx);
                return this.getFuelTankProfile();
            case 1 : 
                return this.getFuelTankProfile();
            case 2 : 
                return this.getFuelTankProfile2();
            default: 
                Print.logWarn("Invalid fuel-tank index: " + ndx);
                return "";
        }
    ...
    **/
    public boolean hasFuelTankProfile(int ndx)
    {
        String flpn = this.getFuelTankProfile(ndx);
        if (FuelLevelProfile.IsNone(flpn)) {
            // -- device FuelLevelProfile is blank/NONE
            return false;
        } else
        if (this.getFuelLevelProfile(ndx,null) == null) {
    ...
    **/
    public FuelLevelProfile getFuelLevelProfile(int ndx, FuelLevelProfile dft)
    {
        String profName = this.getFuelTankProfile(ndx);
    ...
    *** (does not return null)
    **/
    public FuelLevelProfile getFuelLevelProfile(int ndx)
    {
        String profName = this.getFuelTankProfile(ndx);
    ...
    *** Gets the FuelLevelProfile instance for the specified index
    **/
    public double getActualFuelLevel(int ndx, double fuelLevel)
    {
        FuelLevelProfile flp = this.getFuelLevelProfile(ndx); // never null
    ...
    *** @return The last fuel level recieved
    **/
    public double getLastFuelLevel(int ndx)
    {
        switch (ndx) {
            case 0 : return this.getLastFuelLevel();
            case 1 : return this.getLastFuelLevel2();
            default: return -1.0;
        }
    }
    ...
    /**
    *** Sets the last fuel level recieved
    *** @param ndx  fuel level tank index
    *** @param v    The last fuel level recieved
    **/
    public void setLastFuelLevel(int ndx, double v)
    {
        switch (ndx) {
            case 0 : this.setLastFuelLevel(v);  break;
            case 1 : this.setLastFuelLevel2(v); break;
        }
    }
    ...
    **/
    protected boolean _insertEventData(final EventData evdb)
    {
        ...
        */
        ...
        /* fuel level (#1,#2,...) */
        for (int fNdx = 0; fNdx < 2; fNdx++) {
            double fuelLevel = evdb.getFuelLevel(fNdx,true/*estimate*/);
            if (fuelLevel < 0.0) {
                // -- skip (not provided by DCS)
            } else
            if (fuelLevel == 0.0) {
                ...
                this.setLastFuelLevel(fNdx,fuelLevel); // FLD_lastFuelLevel
            }
        }
    ...
    **/
        ...
        throws DBException
    {
        ...
        this._postEventInsertionProcessing();
    }
    ...
    *** Post EventData record insertion processing
    **/
    private void _postEventInsertionProcessing()
    {

        /* ELog/HOS */
        if (this.hasELogHOSProvider() && Device.IsELogEnabled(this)) {
            // -- add ELog/HOS job to thread
            ELogHOSProvider elp = this.getELogHOSProvider();
    ...
    *** Post EventData record insertion processing
    **/
    public void postEventInsertionProcessing()
    {
        this._postEventInsertionProcessing();
    }
    ...
    *** @throws DBException
    **/
    public void calculateStoppedSecondsInRange(final StoppedSeconds stoppedSec[], final boolean checkIgnition)
        throws DBException
    {
                ...
                public int handleDBRecord(EventData ev) throws DBException {
                    ...
                    if (wasMoving.get()) { // true
                        ...
                        if (isMoving) {
                            ...
                            // -- still stopped. "lastStopTime" is last time stopped
                        }
                    }
    ...
    *** @param checkIgnition  True to also check ignition for moving state
    *** @return True if all accumulated stopped seconds are >= their corresponding expected stop secods.
    *** @throws DBException
    **/
    public boolean isExpectedStoppedSeconds(StoppedSeconds stoppedSec[], boolean checkIgnition)
        throws DBException
    {
        int i = this.getFailedExpectedStoppedSecondsIndex(stoppedSec, checkIgnition);
    ...
    *** seconds.
    *** @param stoppedSec     Array of StoppedSeconds
    *** @param checkIgnition  True to also check ignition state for moving condition
    *** @return The index of the failed accumulated stopped-seconds range, or -1 if all pass.
    *** @throws DBException
    **/
    public int getFailedExpectedStoppedSecondsIndex(StoppedSeconds stoppedSec[], boolean checkIgnition)
        throws DBException
    {
        this.calculateStoppedSecondsInRange(stoppedSec,checkIgnition);
        if (ListTools.size(stoppedSec) > 0) {
    ...
    *** @throws DBException
    **/
    public long getStoppedSecondsInRange(long timeStart, long timeEnd, long minStopSec, final boolean checkIgnition)
        throws DBException
    {
      //final boolean checkIgnition = RTConfig.getBoolean(DBConfig.PROP_Device_stoppedInRangeIgnitionOff,STOPPED_IN_RANGE_IGNITION_OFF);
        ...
        this.calculateStoppedSecondsInRange(SS, checkIgnition);
    ...
    **/
    public int checkDriverFatigue(String name, long eventTime, boolean checkIgnition)
    {
        ...
        /* calculate/return */
        try {
            int ndx = dev.getFailedExpectedStoppedSecondsIndex(ssList, checkIgnition);
            if (ndx >= 0) {
                // -- [0..2] true if failed
        ...
        Locale locale, BasicPrivateLabel bpl, Device dev)
    {
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_LEVEL)) {
            if (getTitle) {
                ...
                double level = dev.getLastFuelLevel(); // tank #1
                if (level < 0.0) {
                    return i18n.getString("Device.notAvailable", "n/a");
                } else {
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_VOLUME)) {
            if (getTitle) {
                ...
                double L = dev.getFuelCapacity() * dev.getLastFuelLevel(); // tank #1
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_LEVEL2)) {
            if (getTitle) {
                ...
                double level = dev.getLastFuelLevel2(); // tank #2
                if (level < 0.0) {
                    return i18n.getString("Device.notAvailable", "n/a");
                } else {
                    long pct = Math.round(level * 100.0);
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_VOLUME2)) {
            if (getTitle) {
                ...
                Account.VolumeUnits vu = Account.getVolumeUnits(dev.getAccount());
                double L = dev.getFuelCapacity() * dev.getLastFuelLevel2(); // tank #2
                double V = vu.convertFromLiters(L);
                return StringTools.format(V,"0.0") + " " + vu.toString(locale);
            }
        } else
