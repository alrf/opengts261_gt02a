    ...
    implements EventDataProvider, GeoPointProvider
{
  ...
  //public static final String FLD_appliedPressure      = "appliedPressure";        // kPa
    ...
    **/
    ...
    {
        this.setFieldValue(FLD_humidity, v);
    }
    ...
    **/
    public double getFuelLevelVolume_Liters(int ndx)
    {
        Device device = this.getDevice();
        if (device != null) {
            return this.getFuelLevel(ndx,true/*estimate*/) * device.getFuelCapacity();
        } else {
            return 0.0;
        }
    }
    ...
    *** @return The fuel volume
    **/
    public double getFuelLevelVolume_Units(int ndx)
    {
        Account.VolumeUnits vu = Account.getVolumeUnits(this.getAccount());
        return vu.convertFromLiters(this.getFuelLevelVolume_Liters(ndx));
    }
    ...
    *** @return The estimated fuel-level
    **/
    public double getFuelLevel2(boolean estimate)
    {
        double fuelLevel2 = this.getFuelLevel2(); // no-estimate
        if (!estimate) {
            return fuelLevel2;
        } else
        if (fuelLevel2 > 0.0) {
            ...
            /* fuel-level for tank #2 not supported
            Device device     = this.getDevice(); // should not be null
            double fuelRemain = this.getFuelRemain(false); // no-estimate
            double fuelCapL   = (device != null)? device.getFuelCapacity() : 0.0;
            if ((fuelRemain > 0.0) && (fuelCapL > 0.0)) {
                Print.logDebug("Estimating FuelLevel2 based on device FuelRemaining/FuelCapacity");
                fuelLevel2 = fuelRemain / fuelCapL;
                if (fuelLevel2 > 1.0) { fuelLevel2 = 1.0; }
                return fuelLevel2;
            } else {
                return 0.0;
            }
    ...
    /**
    *** Gets the fuel level in the alternate/second tank
    *** @param ndx  The fuel tank index
    *** @return The fuel level in the alternate/second tank
    **/
    public double getFuelLevel(int ndx)
    {
        switch (ndx) {
            case 0 : return this.getFuelLevel();
            case 1 : return this.getFuelLevel2();
            default: return -1.0;
        }
    }
    ...
    *** @param v    The fuel level in the alternate/second tank
    **/
    public void setFuelLevel(int ndx, double v)
    {
        switch (ndx) {
            case 0 : this.setFuelLevel(v);  break;
            case 1 : this.setFuelLevel2(v); break;
        }
    }
    ...
    *** @return The estimated fuel-level
    **/
    public double getFuelLevel(int ndx, boolean estimate)
    {
        switch (ndx) {
            case 0 : return this.getFuelLevel(estimate);
            case 1 : return this.getFuelLevel2(estimate); // estimate not support for fuelLevel2
            default: return -1.0;
        }
    }
    ...
    **/
    public double getFuelDeltaPercent(int tankNdx, long secAgo)
    {
        EventData ed = this;

        /* get current fuel level */
        double currentFuelLevel = ed.getFuelLevel(tankNdx,true/*estimate*/); // #1
        ...
        if (secAgo <= 0L) {
            ...
            lastFuelLevel = dev.getLastFuelLevel(tankNdx); // #1
        } else {
            long ts = ed.getTimestamp() - secAgo; 
            try {
                boolean validGPS = false;
                EventData pev = dev.getFirstEvent(ts, validGPS);
                lastFuelLevel = (pev != null)? pev.getFuelLevel(tankNdx) : -1.0; // #1
            } catch (DBException dbe) {
    ...
    **/
    public boolean isFuelLevelDrop(int tankNdx, double pctDeltaWhileStopped, double pctDeltaWhileMoving, long secAgo)
    {
        EventData ed = this;
        ...
        /* check for movement, based on odometer delta */
    ...
    public double getFuelRemain(boolean estimate)
    {
        ...
        if (!estimate || (fuelRemain > 0.0)) {
            ...
            if ((fuelLevel > 0.0) && (fuelCapL > 0.0)) {
                fuelRemain = fuelCapL * device.getActualFuelLevel(1,fuelLevel);
                return fuelRemain;
            } else {
                return 0.0;
            }
        ...
        Locale locale, BasicPrivateLabel bpl, EventData ed)
    {
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_LEVEL2)) {
            if (getTitle) {
                ...
                double level = ed.getFuelLevel2(true/*estimate*/); // estimate not support for tank #2
                if (level < 0.0) {
                    return i18n.getString("EventData.notAvailable", "n/a");
                } else {
                    long pct = Math.round(level * 100.0);
